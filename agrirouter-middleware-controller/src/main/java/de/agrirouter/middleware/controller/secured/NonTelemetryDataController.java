...
public class NonTelemetryDataController implements SecuredApiController {
    ...
    /**
     * Publish a task data message for a given endpoint.
     *
    ...
    public ResponseEntity<Void> publish(@Parameter(description = "The external endpoint ID.", required = true) @PathVariable String externalEndpointId,
                                        @Parameter(description = "The request body containing all necessary information to publish a task data message.", required = true) @Valid @RequestBody PublishNonTelemetryDataRequest publishNonTelemetryDataRequest) {
