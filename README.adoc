...
- Body: JSON object as shown below (replace the placeholders with your values)
...
|versionId |string |Yes |The certification version ID for the application variant you are registering.
...
|routerDevice |object |Yes |Router device object downloaded from the agrirouter© developer portal (see structure below).
...
|authentication.type |enum |Yes |Certificate type (depends on the certificate you selected when creating the router device; e.g., PEM or P12).
|authentication.secret |string |Yes |Password/secret protecting the certificate if applicable.
|authentication.certificate |string |Yes |Certificate content as delivered by the agrirouter© (paste the content as a single string).
...
|technicalMessageType |enum |Yes |One of the values supported by the middleware. See the list in the Functional Documentation under "Define the capabilities of the application" for all available values (e.g., `ISO_11783_TASKDATA_ZIP`, `SHP_SHAPE_ZIP`, `DOC_PDF`, `IMG_JPEG`, `IMG_PNG`, `IMG_BMP`, `VID_AVI`, `VID_MP4`, `VID_WMV`, `GPS_INFO`, `ISO_11783_DEVICE_DESCRIPTION`, `ISO_11783_TIME_LOG`).
|direction |enum |Yes |How the app handles the message type. Allowed values: `SEND`, `RECEIVE`, `SEND_RECEIVE`.
|===

Note:

- All required fields are validated by the API; missing or invalid values result in HTTP 400 responses.
